...
const orgData = [
  // Üst yönetim
  { id: 1,  name: "Mehmet Tankut",        title: "Şirket Müdürü",              parentId: null },
...
const BRAND = "#005e95"; // Şirket Müdürü / CEO
const DEPT_COLORS = {
...
function avatarIcon(dept) {
  const inner = (dept && DEPT_ICONS[dept]) || LEADER_ICON;
...
function renderNode(node, { isRoot = false, accent = BRAND, dept = null } = {}) {
  // Departman başıysa kendi rengini/departmanını benimser, değilse üstten miras alır
  const myAccent = node.dept ? (DEPT_COLORS[node.dept] || accent) : accent;
  ...
  card.className = "card" + (isRoot ? " card--root" : "");
  ...
  avatar.innerHTML = avatarIcon(myDept);
  ...
  if (node.children.length) {
    const vertical = isVerticalGroup(node);
    if (vertical) {
    ...
    childrenBox.className = "children" + (vertical ? " children--vertical" : "");
    node.children.forEach(child =>
      childrenBox.appendChild(renderNode(child, { accent: myAccent, dept: myDept }))
    );
...
function highlightDept(name) {
  ...
  cardById.forEach((card, id) => {
    const match = accentById.get(id) === color;
